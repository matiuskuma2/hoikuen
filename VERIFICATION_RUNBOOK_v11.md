...
| ID | 区分 | 検証内容 | 判定 | 証拠 |
|----|------|----------|------|------|
| K1 | Bug Fix | CSV再インポートで重複行が増えない | ⬜ | |
| K2 | Bug Fix | DB取込でWorker上限エラーが出ない | ⬜ | |
| K3 | Bug Fix | 月間予定表が正しく表示される | ⬜ | |
| K4 | Bug Fix | 請求Excelに一時保育児童が含まれる | ⬜ | |
| K5 | Bug Fix | 延長保育が7:00-7:30 / 20:00-21:00のみ | ⬜ | |
| K6 | Bug Fix | 日報Excelに大学提出用2シートがある | ⬜ | |
| K7 | New Feature | 保護者入力URL(/my/:token)で予定提出可能 | ⬜ | |
| K8 | New Feature | 職員日報画面が表示・印刷可能 | ⬜ | |
...
### 判定
- **結果**: ⬜ 未検証
- **証拠**: （APIレスポンスのスクリーンショット or JSON貼付）
- **メモ**:
...
### 判定
- **結果**: ⬜ 未検証
- **証拠**: （usage_facts クエリ結果 + Excelスクリーンショット）
- **メモ**:
