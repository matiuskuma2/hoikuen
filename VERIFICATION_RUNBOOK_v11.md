...
**作成日**: 2026-03-18  
**最終更新**: 2026-03-18 (K3/K7/K8を「木村さん確認待ち」に変更 + E2E通し確認完了 + breakfast_flag修正 + 月バリデーション追加)  
**対象バージョン**: v11.0  
**検収責任者**: 木村さん（最終判定）  
...
| K3 | Bug Fix | 月間予定表が正しく表示される | 🟡 **木村さん確認待ち** | **中** — API検証済、テストデータでの月切替動作済。木村さんの実データでの画面表示・月切替を未確認 |
...
| K7 | New Feature | 保護者入力URL(/my/:token)で予定提出可能 | 🟡 **木村さん確認待ち** | **中** — API動作+フロントJS検証済。ブラウザ上での編集操作・月切替・保存ボタン押下・スマホでの使いやすさは木村さん確認待ち |
| K8 | New Feature | 職員日報画面が表示・印刷可能 | 🟡 **木村さん確認待ち** | **低** — HTTP 200+HTML構造+印刷CSS確認済。データが実際に画面表示されるか・A4印刷レイアウトの確認は木村さん確認待ち |

**判定記号**: ✅ PASS / ❌ FAIL / ⬜ 未検証 / ⚠️ 条件付きPASS / 🟡 木村さん確認待ち
...
### K3: 月間予定表 — 🟡 木村さん確認待ち (API検証済、画面操作は未確認)
...
### K7: 保護者入力URL — 🟡 木村さん確認待ち (API+フロントJS検証済、実機操作は未確認)
...
### K8: 職員日報画面 — 🟡 木村さん確認待ち (HTML+CSS+エラーなし確認済、実表示・印刷は未確認)
...
| ステータス | 詳細 |
|:---------:|:----:|
| 🟡 **木村さん確認待ち** | K3/K7/K8 は技術検証済・木村さんの実機・実データでの最終確認待ち |
...
お疲れさまです。v11.0の検収状況をご報告します。

【確認済み（技術検証PASS・証拠あり）】
✅ K1: CSV再インポートで重複なし
✅ K2: DB取込でエラーなし（59児童×20日＝1,180件処理）
✅ K4: 請求Excelに一時保育児童が含まれている
✅ K5: 延長保育が7:00-7:30 / 20:00-21:00のみ適用
✅ K6: 日報Excelに大学提出用2シートあり

【木村さんの確認が必要な項目（3件）】
以下の順番で確認をお願いできますでしょうか。

①（最優先）園児CSVインポート → ルクミー登降園データ取込 → 利用予定取込 → ダッシュボード表示
  → 一連のアップロード〜表示が途中で止まらないことの確認

②（第2優先）保護者入力画面（/my/{view_token}）
  → スマホで開いて、月切替 → 日付タップ → 時間入力 → 保存 → リロードして反映確認

③（第3優先）職員日報画面（/staff/daily/2026/3/1）
  → PCブラウザで開いて、データ表示確認 → Ctrl+P で印刷プレビューがA4で崩れないか

※ 前回の確認では、テスト手順のミス（APIのURLパターン・フィールド名の誤り）があり、
  一部結果が実態と異なっていました。修正済みです。
※ 帳票Excel精密化・大学提出最終版・PDF対応は後回しにしています。

確認環境:
- Sandbox: https://3000-i726fppspidcgasv3kn1s-d0b9e1e2.sandbox.novita.ai
- 本番: https://ayukko-prod-2fx.pages.dev/
```
...
**K3/K7/K8 全てOKの場合:**
```
確認ありがとうございます。K1-K8 全項目 PASS として
v11.0 完了報告を正式に行います。
```
...
```
確認ありがとうございます。
NG項目（K○）について原因を調査し、修正後に再確認を依頼します。
修正見込み: [時間見積もり]
```
...
| 優先度 | 内容 | ステータス |
|--------|------|-----------|
| **最優先** | アップロード系（CSV・登降園・予定）が全て止まらずに通ること | 🟡 木村さん実データで最終確認待ち |
| **第2優先** | 保護者がLINE/URLから予定を入れて保存・再表示できること | 🟡 スマホ実機テスト待ち |
| **第3優先** | 職員が誰の予定が入っているか日報画面で見て印刷できること | 🟡 ブラウザ表示+印刷テスト待ち |
| いったん止め | 請求Excel精密化、日報Excel大学提出最終版、一括生成、PDF | ⏸️ 上の3つが完了後に着手 |
