...
**作成日**: 2026-03-18  
**最終更新**: 2026-03-18 (E2E通し確認完了 + breakfast_flag修正 + 月バリデーション追加)  
**対象バージョン**: v11.0  
**検収責任者**: 木村さん（最終判定）  
**ステータス**: 🟡 **サンドボックスE2E通し確認完了 — 木村さん実データ・実機での最終確認待ち**

> **追加修正 (2026-03-18)**:
> - view APIに`breakfast_flag`が含まれていなかったバグを修正
> - ダッシュボードAPIに月1-12のバリデーションを追加
> - 保護者入力→DB保存→職員ダッシュボード閲覧のE2E通し確認を同一データで実施・成功
...
| **現在のDB状態** | 園児61名 (月極19 / 一時42) — テストデータ含む |

---

## 方針（モギモギさん指示に基づく優先順位）

**第1優先**: アップロードエラーを完全になくす（園児CSV・登降園・利用予定表の取込→ダッシュボード表示）  
**第2優先**: LINE/URLから予定を入れられる（保護者入力→保存→再表示→修正→月切替）  
**第3優先**: 職員側で確認できる（誰が予定出したか→日報画面で見られるか→印刷）

**いったん止める範囲**: 請求Excel精密化、日報Excel大学提出最終版、一括生成完成、PDF最終対応

---

## E2E通し確認結果（2026-03-18 同一データで一気通貫）

| Step | 内容 | 結果 |
|------|------|------|
| 1. 保護者入力 | 山田太郎 4月6-8日をsubmit API経由で入力 | upserted=3 ✅ |
| 2. DB保存 | schedule_plansテーブルに3レコード | breakfast_flag含め正確 ✅ |
| 3. 修正 | Day 6を08:30→07:30, 17:00→18:30に変更 | upserted=3, 永続化確認 ✅ |
| 4. 月切替 | 3月(20日)→4月(3日)→5月(0日) | 各月正しく返却 ✅ |
| 5. 職員ダッシュボード | 4月ダッシュボードに山田太郎が07:30-18:30で表示 | ✅ |
| 6. 職員日報ページ | /staff/daily/2026/4/6 HTTP 200 | ✅ |
